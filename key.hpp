...
static constexpr int C=4;  // number of bits per character
static constexpr int N=20; // size of alphabet (2^B)
#define C_IS_SET
...
std::string s;
inline std::string Key::convertToString() const {
	s = "";
	std::for_each(m_digit.begin(), m_digit.end(), [](const char& c)
	{
		s = s + ALPHABET[c];
